jQuery(document).ready(function($) {
  ...
  var prepareGallery = function() {
    if (!overlay) {
      ...
      imageMeta = $("<div></div>")
        ...
        .css({ float: "right", "margin-top": "20px", width: "250px" });
      imageMeta
        .append(buttons)
        .append(
      ...
      titleAndDescription = $("<div></div>")
        ...
        .css({ width: "100%", "margin-top": imageMeta.css("margin-top") });
      ...
      comments = $("<div></div>")
        ...
        .css({ width: "100%", bottom: "10px", "margin-top": "20px" });
      var commentsLoading = $(
      ...
      var leftWidth =
        $(window).width() - screenPadding * 2 - (imageMeta.width() + 40);
      leftWidth += "px";
      ...
      var targetBottomPos =
        $(window).height() - parseInt(info.css("top"), 10) + "px";
      ...
      container
        ...
        .click(function(e) {
          ...
          if (
            target.is(gallery) ||
            target.parents().add(target).is(close_hint)
          ) {
            container.jp_carousel("close");
            ...
            $("#jp-carousel-comment-form-submit-and-info-wrapper").slideDown(
              "fast"
            );
            ...
            target.parents("#jp-carousel-comment-form-container").length
          ) {
            ...
            if (textarea && textarea.attr("id") === target.attr("id")) {
              ...
              if (1 !== Number(jetpackCarouselStrings.is_logged_in)) {
                ...
                if (1 === Number(jetpackCarouselStrings.require_name_email)) {
                  if (
                    !ajaxData["email"].length ||
                    !ajaxData["email"].match("@")
                  ) {
                    gallery.jp_carousel("postCommentError", {
        ...
        .on("transitionend.jp-carousel ", ".jp-carousel-slide", function(e) {
          if ("transform" === e.originalEvent.propertyName) {
            var transitionMultiplier =
              (Date.now() - transitionBegin) /
              1000 /
              e.originalEvent.elapsedTime;
            container.off("transitionend.jp-carousel");
            if (transitionMultiplier >= 2) {
              $(".jp-carousel-transitions").removeClass(
  ...
  var methods = {
    ...
    updateSlidePositions: function(animate) {
      ...
      if (1 === direction) {
        if (!nextNext.is(previous)) {
          nextNext
            .jp_carousel("setSlidePosition", galleryWidth + next.width())
            .show();
        }
        if (!previousPrevious.is(next)) {
          previousPrevious
            .jp_carousel(
              "setSlidePosition",
              -previousPrevious.width() - currentWidth
            )
            .show();
        }
      } else {
        if (!nextNext.is(previous)) {
          nextNext
            .jp_carousel("setSlidePosition", galleryWidth + currentWidth)
            .show();
        }
      }
      previous
        .jp_carousel(
          "setSlidePosition",
          Math.floor(-previous.width() + screenPadding * 0.75)
        )
        .show();
      next
        .jp_carousel(
          "setSlidePosition",
          Math.ceil(galleryWidth - screenPadding * 0.75)
        )
        .show();
    },
    selectSlide: function(slide, animate) {
      ...
      window.location.hash = last_known_location_hash =
        "#jp-carousel-" + attachmentId;
    },
    slides: function() {
    ...
    fitSlide: function() {
      return this.each(function() {
        ...
        dimensions.left = 0;
        dimensions.top =
          Math.floor((max.height - dimensions.height) * 0.5) + 40;
    ...
    initSlides: function(items, start_index) {
      ...
      items.each(function(i) {
        var src_item = $(this),
          ...
          caption =
            src_item.parents(".gallery-item").find(".gallery-caption").html() ||
            "",
    ...
    selectBestImageSize: function(args) {
      ...
      if (
        "undefined" === typeof args.orig_width ||
        "undefined" === typeof args.max_width
      ) {
        return args.orig_file;
      }
      if (
        "undefined" === typeof args.medium_file ||
        "undefined" === typeof args.large_file
      ) {
        return args.orig_file;
      }
      ...
      var isPhotonUrl =
        imageLinkParser.hostname.match(/^i[\d]{1}.wp.com$/i) != null;
      ...
      if (isPhotonUrl) {
        ...
        if (-1 !== largeFileIndex) {
          ...
          if (
            args.orig_width > args.max_width ||
            args.orig_height > args.max_height
          ) {
            origPhotonUrl +=
              "?fit=" + args.orig_max_width + "%2C" + args.orig_max_height;
          }
        }
    ...
    getTitleDesc: function(data) {
      ...
      if (title.length || desc.length) {
        if (
          $("<div />").html(title).text() === $("<div />").html(desc).text()
        ) {
          title = "";
        }
        markup = title.length
    ...
    updateExif: function(meta) {
      ...
      $.each(meta, function(key, val) {
        ...
        $ul.append(
          "<li><h5>" + jetpackCarouselStrings[key] + "</h5>" + val + "</li>"
        );
      });
      $("div.jp-carousel-image-meta ul.jp-carousel-image-exif").replaceWith(
        $ul
      );
    },
    updateFullSizeLink: function(current) {
      ...
      var permalink = $(
        ...
        .attr("href", original)
        .attr("target", "_blank");
      $("div.jp-carousel-image-meta a.jp-carousel-image-download").replaceWith(
        permalink
      );
    },
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          if (
            current &&
            current.data &&
            current.data("attachment-id") != args.attachment_id
          ) {
    ...
    setCommentIframeSrc: function(attachment_id) {
      ...
      if (iframe && iframe.length) {
        iframe.attr(
          "src",
          iframe.attr("src").replace(/(postid=)\d+/, "$1" + attachment_id)
        );
        iframe.attr(
          "src",
          iframe
            .attr("src")
            .replace(/(%23.+)?$/, "%23jp-carousel-" + attachment_id)
        );
      }
    },
    ...
    nextSlide: function() {
      ...
      if (
        selected.length === 0 ||
        (slides.length > 2 && selected.is(slides.last()))
      ) {
        return slides.first();
      }
    ...
    prevSlide: function() {
      ...
      if (
        selected.length === 0 ||
        (slides.length > 2 && selected.is(slides.first()))
      ) {
        return slides.last();
      }
    ...
    loadFullImage: function(slide) {
      ...
      if (!image.data("loaded")) {
        ...
        if (
          ...
          (slide.data("thumb-size") &&
            slide.width() > slide.data("thumb-size").width)
        ) {
          image
          ...
          image
            .attr("src", slide.data("preview-image"))
            .attr("itemprop", "image");
        }
        image.data("loaded", 1);
      }
  ...
  $(
    ...
    "click.jp-carousel",
    "div.gallery,div.tiled-gallery, a.single-image-gallery",
    function(e) {
      ...
      $(this).jp_carousel("open", {
        start_index: $(this)
          ...
          .index($(e.target).parents(".gallery-item, .tiled-gallery-item"))
      });
    }
  );
  ...
  $(window).on("hashchange.jp-carousel", function() {
    ...
    galleries.each(function(_, galleryEl) {
      ...
      if (selectedThumbnail) {
        $(selectedThumbnail.gallery).jp_carousel(
          "openOrSelectSlide",
